...
- `enquest_taxonomy.json` — the classification rules (15 Hazard labels, 11 Human Factors labels, keyword triggers, precedence rules). Editable without a code change.
- `enquest_prompt.py` — builds the Agent instructions text and a reference JSON schema from the taxonomy file.
- `enquest_classifier_client.py` — calls the Azure AI Foundry Agent (`EnQuestObservationClassifier`) via the OpenAI Responses API and parses its response.
- `enquest_sql_engine.py` — all `dbo.FormData_Master` access: adds the `AI_Hazard_Category` / `AI_Human_Factors_Category` columns if missing, fetches unclassified cards, writes results back, and logs Low-confidence results to `dbo.AI_Classification_Review_Log`.
- `enquest_process_batch.py` — classifies a batch of cards concurrently, with a bounded confidence-retry (a card is retried up to 2 extra times if Low confidence, then persisted regardless and flagged for review if still Low).
- `enquest_classify.py` — the shared `classify_observations(force, limit, concurrency)` function both entry points call.
- `enquest_main.py` — **this is the daily webjob entry point.** Runs `classify_observations(force=False)`, i.e. only classifies cards not yet classified. Safe to run daily indefinitely.
- `enquest_backfill.py` — the manual/on-demand entry point used for the historical run (not needed for the daily job, but useful if a taxonomy change requires a `--force` reclassify later).
...
1. Deploy the repo (or at minimum: all `enquest_*.py` files, `enquest_taxonomy.json`, `requirements.txt`) to the App Service, under:
   `%HOME%\site\wwwroot\App_Data\jobs\triggered\EnQuest-BBSS-Classification-Webjob\EnQuest_BBSS_Classification_WebJob\`
   (this exact path is hardcoded in `enquest_run.cmd` — either deploy to this path or update the `JOB_PATH` variable in `enquest_run.cmd` to match wherever it actually lands).
2. `enquest_run.cmd` is the WebJob's run command — it installs `requirements.txt` on each run and then calls `enquest_main.py`. No changes needed unless the deployment path differs from the default above.
3. `enquest_settings.job` sets the schedule: `{"schedule":"0 0 6 * * *"}` — daily at 06:00 UTC (NCRONTAB format: second, minute, hour, day, month, day-of-week). Adjust the hour if a different time is preferred.

### 3.4 Verify the deployment

1. In the Azure Portal → App Service → **WebJobs**, confirm `EnQuest-BBSS-Classification-Webjob` shows up and is scheduled.
...
- **Taxonomy changes**: edit `enquest_taxonomy.json`, run `python enquest_agent_setup.py` locally, paste the new instructions into the Foundry portal's Agent configuration, then run `enquest_backfill.py --force` once to reclassify historical data under the new rules if needed. The daily webjob itself needs no changes for this.
